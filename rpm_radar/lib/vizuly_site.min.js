...
function initialize() {
    ...
    }).interpolate("cardinal-closed").on("validate", onValidate).on("mouseover", onMouseOver).on("mouseout", onMouseOut).on("measure", onMeasure), theme = vizuly.theme.radial_linearea(viz).skin(vizuly.skin.LINEAREA_OCEAN), viz_title = viz.selection().select("svg").append("text").attr("class", "title").attr("y", 35).attr("text-anchor", "middle").text("RPM"), viz_heading = viz.selection().select("svg").append("text").attr("class", "heading").attr("y", 55).attr("text-anchor", "middle").text(""), changeSize(d3.select("#currentDisplay").attr("item_value"))
}
