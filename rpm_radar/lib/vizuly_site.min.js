/*! vizuly 28-04-2016 */
var keys = ['PG', 'SG', 'SF', 'PF', 'C']
var keysLength = keys.length,
    selectedTeams = [29],
    teamNames = ['GSW', 'HOU', 'TOR', 'MIN', 'BOS', 'OKC', 'PHI', 'WAS', 'NOP', 'CLE', 'SAS', 'DEN', 'DET', 'IND', 'MIA', 'UTA', 'MIL', 'POR', 'DAL', 'CHA', 'LAC', 'NYK', 'BKN', 'MEM', 'ATL', 'LAL', 'CHI', 'ORL', 'PHX', 'SAC'],
    TABLE_HEADERS = ['Name'];
...
function drawTable() {
    ...
    tHead.selectAll('th')
        ...
        .on('click', function (d, i) {
            ...
            d3.select('div#viz_container').html("")
            loadData();
        })
    ...
    var rows = table.selectAll('tr').filter('.team-row')
        ...
        .on("mouseover", function (d, i) {
            ...
            //     if (document.getElementById(d + '-' + key).dispatchEvent(evt)) {
            //         console.log(d + '-' + key)
            //     }
            // });
        })
...
function loadData() {
    d3.csv("data/rpm.csv", function (csv) {
        var a = [];
        selectedTeams.forEach(function (i) {
            d = csv[i]
            keys.forEach(function (key, j) {
                var value = parseFloat(d[key].replace('%', ''))
                a.push({
                    'type': key,
                    'name': d.RPM,
                    'value': value,
...
function initialize() {
    viz = vizuly.viz.corona(document.getElementById("viz_container")), viz.data(data).margin({
            ...
            return a.type
        }).interpolate("cardinal-closed").on("validate", onValidate).on("mouseover", onMouseOver).on("mouseout", onMouseOut).on("measure", onMeasure), theme = vizuly.theme.radial_linearea(viz).skin(vizuly.skin.LINEAREA_OCEAN),
        viz_title = viz.selection().select("svg").append("text").attr("class", "title").attr("y", 40).attr("text-anchor", "middle").text("2017-18 赛季 RPM 阵容深度（时间加权）"),
        viz_heading = viz.selection().select("svg").append("text").attr("class", "heading").attr("y", 300).attr("text-anchor", "middle").text("@篮圈即是原点"), changeSize(d3.select("#currentDisplay").attr("item_value"))
}

function onValidate() {
    viz.xAxis().tickFormat(function (a) {
        return convertIndex(a)
    }), viz.yAxis().tickFormat(function (a, i) {
        return a + "%"
    }), viz.xAxis().ticks(keysLength), viz.yAxis().ticks(6)
}

function onMeasure() {
    viz_title.attr("x", viz.width() / 2)
        .attr("y", viz.height() * 0.07)
        .style("font-size", function () {
            return viz.height() / 40
        }).style("fill", function () {
            return theme.skin().labelColor
        }), viz_heading
        .attr("x", viz.width() - viz.height() / 10)
        .attr("y", viz.height() * 0.98)
        .style("font-size", function () {
            return viz.height() / 45
        }).style("fill", function () {
            return theme.skin().labelColor
        }).style("fill-opacity", .5).style("")
}
...
function onMouseOver(a, b) {
    ...
    d.selectAll(".vz-tip-path").remove(), e.selectAll(".my-tip").remove(), d.append("path").attr("class", "vz-tip-path").attr("id", function (a, b) {
            ...
            return "Better than: " + viz.yAxis().tickFormat()(viz.y()(a)) + " Teams"
...
vizuly.version = "1.0", vizuly.ui = {}, vizuly.viz = {}, vizuly.viz.layout = {}, vizuly.viz.layout.CLUSTERED = "CLUSTERED", vizuly.viz.layout.STACKED = "STACKED", vizuly.viz.layout.OVERLAP = "OVERLAP", vizuly.viz.layout.STREAM = "STREAM", vizuly.color = {}, vizuly.color.shift = function (a, b) {
    ...
    function c() {
        if (i) {
            ...
            b.attr("class", i["class"]), b.selectAll(".vz-background").attr("fill", function () {
                ...
                }), b.selectAll(".vz-data-point").style("opacity", 0),
                b.selectAll(".vz-radial-x-axis-tick")
                .style("font-weight", i.xAxis_font_weight)
                .style("fill", i.labelColor)
                .style("font-weight", 300)
                .style("fill-opacity", .8)
                .style("font-size", Math.max(16, Math.round(a.outerRadius() / 20)) + "px"),
                b.selectAll(".vz-y-axis-tick").style("stroke", i.yAxis_line_stroke).style("stroke-width", 1).style("opacity", i.yAxis_line_opacity), b.selectAll(".vz-y-axis-tick-label").style("font-size", Math.max(8, Math.round(a.outerRadius() / 30)) + "px").style("fill", i.labelColor).style("font-weight", 200).style("fill-opacity", function () {
    ...
    function d() {
        ...
        a.enter().append("g").attr("class", "vz-series-plots"), a.exit().remove(), a.each(function (a, b) {
                ...
                var c = w.selectAll("vz-tip").data(a).enter().append("g").attr("class", "vz-tip").attr('id', function (d) {
