...
function initialize() {
    viz = vizuly.viz.corona(document.getElementById("viz_container")), viz.data(data).margin({
        ...
        viz_title = viz.selection().select("svg").append("text").attr("class", "title").attr("y", 50).attr("text-anchor", "middle").text("2017-18 赛季 RPM 阵容深度（时间加权）截至1/15"),
        viz_heading = viz.selection().select("svg").append("text").attr("class", "heading").attr("y", 25).attr("text-anchor", "middle").text("@篮圈即是原点"), changeSize(d3.select("#currentDisplay").attr("item_value"))
}
